...
These documents can be found in the same folder als this one.

### Local gRPC connection

By default the API listens on port `11111` without authentication and encryption.

Each endpoint only accommodates one concurrent connection at the same time.
Multiple connections are possible, but responses to any ongoing process are always sent to the newest connection only.
