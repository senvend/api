...
/// Implements conversions between a protobuf UUID struct and `uuid::Uuid`.
/// The conversion will fail, if the UUID version is not 4 (random), with Error:
/// senvend_api_utils::UuidVersionError.
///
/// NOTE: To use this macro, it is required that the `senvend_api_utils` crate is included as a
/// dependency!!
#[proc_macro_derive(ProtoUuid4)]
pub fn derive_proto_uuid4_conversion(item: TokenStream) -> TokenStream {
    ...
    let code = quote! {
        impl #impl_generics TryFrom<&#name> for uuid::Uuid #ty_generics #where_clause {
            type Error = senvend_api_utils::UuidVersionError;
            fn try_from(value: &#name) -> Result<Self, Self::Error> {
                let uuid = uuid::Uuid::from_u64_pair(value.msb, value.lsb);
                match uuid.get_version() {
                    Some(uuid::Version::Random) => Ok(uuid),
                    Some(v) => Err(senvend_api_utils::UuidVersionError::InvalidVersion(v)),
                    None => Err(senvend_api_utils::UuidVersionError::MissingVersion),
                }
            }
        }
